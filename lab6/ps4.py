...
make_one_curve_plot(
    range(300), aveList, "Timestep", "Average Population", "Without Antibiotic"
)
        ...
        if (self.birth_prob * (1 - pop_density)) >= random.random():
            if self.get_resistant():
                ...
                if resistant_pop >= random.random():
                    return ResistantBacteria(
                        self.birth_prob, self.death_prob, True, self.mut_prob
                    )
                else:
                    return ResistantBacteria(
                        self.birth_prob, self.death_prob, False, self.mut_prob
                    )
        else:
